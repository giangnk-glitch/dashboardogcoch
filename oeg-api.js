...
const OEG = (() => {
  ...
  function getAuthSessionToken() {
    try { return window.OEGAuth?.getSession?.()?.token || null; } catch(e) { return null; }
  }

  function getAuthSessionUser() {
    try { return window.OEGAuth?.getUser?.() || null; } catch(e) { return null; }
  }

  function activeToken() { return _token || getAuthSessionToken(); }
  ...
  function getUser() { return _user || getAuthSessionUser(); }
  function isLoggedIn() { return !!activeToken(); }
  ...
  async function get(module, options = {}) {
    ...
    const url = `${API_URL}?action=${action}&module=${module}&branch=${encodeURIComponent(branch)}&period=${period}&token=${token}`;
